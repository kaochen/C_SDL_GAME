...
int displayTopBar (int levelNumber, SDL_Surface * screen,
		    ...
		    S_LevelList * levelList, Square grid[][getMax_Y_Blocks ()]);
...
int goalLeft (Square grid[][getMax_Y_Blocks ()]);

/* count goals all ready achieve */
int nbr_of_goals (Square grid[][getMax_Y_Blocks ()]);

/* Display Progress in the level */
int displayProgress (Square grid[][getMax_Y_Blocks ()], SDL_Surface * menu,
		      Sprites tableSurface[NBR_OF_IMAGES]);

/* Victory or not ? */
int levelFinished (Square grid[][getMax_Y_Blocks ()]);
